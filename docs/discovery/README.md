...
```
docs/discovery/<need>.md  ──►  Go / No-Go / Pivot  ──►  on "Go": create docs/initiative/<name>/
```

## How to run a discovery

1. Copy the template: `docs/template/discovery.md` → `docs/discovery/<need-name>.md`
...
- **Go** → create an initiative: copy `docs/template/_initiative/` to
  `docs/initiative/<name>/`, and link *this* discovery from the initiative README + PRD gate.
...
`docs/discovery/` means the qualification trail — including the rejections — survives regardless
of what got built.
